...
*/
  ...
  t = e.send(get_frame(0x13, 3, 0x20C1, 0x00, 8, 0)); // total number of bytes to write ... is it 16bit or 8bit ?
  if(t) { 
...
*/  }
  ...
  int ls[11];
  ls[0] = 0x1503; // opcode len-1(WORDS)

//  ls[1] = 0x0007;// + (1<<6); // res toggle(0x0040) len-1(BYTES)
  
  ls[1] = 2000;//0x0000; // LSBpos
  ls[2] = 0x0000; // MSBpos
  ls[3] = 1260;//0x0000; // LSBvel
  ls[4] = 200<<8; // TIME MSBvel
  ...
  ls[7] = 0x0000; // MSBpos
  ls[8] = 0x0000; // LSBvel
  ls[9] = 0x0000; // TIME MSBvel
  */
  ls[10] = 0;
  uint16_t crc = e.get_crc(ls, 6);
...
//  Serial.println(crc, HEX);
  ...
  Serial1.write(0x03); // len rs232
//  Serial1.write(ls[1]); // control byte
//  Serial1.write(ls[1]>>8);
 
  Serial1.write(ls[1]); // data 0
  Serial1.write(ls[1]>>8);
  Serial1.write(ls[2]); // data 1
  Serial1.write(ls[2]>>8);
  Serial1.write(ls[3]); // data 2
  Serial1.write(ls[3]>>8);
  Serial1.write(ls[4]); // data 3
  Serial1.write(ls[4]>>8);

/*  Serial1.write(0x00); // data 0
  Serial1.write(0x00);
  Serial1.write(0x00); // data 1
  Serial1.write(0x00);
  Serial1.write(0x00); // data 2
  Serial1.write(0x00);
