...
uint16_t EPOS2::get_drive_modes() {
	/*	bit val description
		31...22 0 reserved
...
int EPOS2::set_max_acc(uint32_t a) {
	int t;
	t = send(get_frame(WRITE_OPCODE, 2, 0x60C5, 0x00, (uint16_t) a, (uint16_t) a>>16));
	if(t) return -1;
	epos2_frame f;
	t = get(&f);
	if(t) return -1;
...
uint32_t EPOS2::get_max_acc() {
	int t;
	t = send(get_frame(READ_OPCODE, 2, 0x60C5, 0x00, 0, 0));
	if(t) return -1;
	epos2_frame f;
	t = get(&f);
	if(t) return -1;
...
uint8_t EPOS2::get_operation_mode() {
	int t;
